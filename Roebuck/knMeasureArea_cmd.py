...
def calculate_area_with_units(geos, target_unit_name="SF"):
    ...
    # Handle the "Current" or unsupported cases
    ...
    # Special case for Acres (calculate in SF then divide by 43560)
    if target_unit_name.upper() == "ACRE":
        scale_to_feet = Rhino.RhinoMath.UnitScale(doc_unit_system, Rhino.UnitSystem.Feet)
        area_factor = (scale_to_feet * scale_to_feet) / 43560.0
    
    for geo in geos:
        rhobj = rs.coercerhinoobject(geo, True, True)
        if rhobj:
            # Handle Brep/Surface/Mesh geometries
            amp = rg.AreaMassProperties.Compute(rhobj.Geometry)
            if amp:
                total_area += (amp.Area * area_factor)
            else:
                # Fallback for closed curves if AreaMassProperties fails on geometry directly
...
# Prompt for geometries
    geos = rs.GetObjects("Select geometries for area measurement", 
    ...
    rs.ClipboardText(formatted_area.replace(",", "")) # Raw number for clipboard
