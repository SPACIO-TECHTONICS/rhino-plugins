...
# RHINO CHARTS v1.0.99 STARTUP STAMP
...
from assets.overlay_window import ChartOverlay
...
class RhinoChartsDashboard(forms.Form):
    ...
    def __init__(self):
        ...
        # Initialize Display Conduit
        self.conduit = ChartDisplayConduit()
        
        # Subscribe to selection events immediately
    ...
    def setup_ui(self):
        ...
        # Load index.html
        # Since charts_panel is in dev/assets, we need to go up one level to dev/
        ...
        if os.path.exists(html_path):
            ...
            Rhino.RhinoApp.WriteLine("RhinoCharts ERROR: Dashboard resource not found at: " + html_path)
    ...
    def OnSelectObjects(self, sender, e):
        # Visible feedback in Rhino Console
        ...
        for obj in obj_ids:
            try:
                ...
                pass
            if len(data_list) >= 20: break # Practical limit for readability
    ...
    def update_browser(self, data):
        ...
        try:
            ...
            script = "if(window.updateChart) {{ window.updateChart({}); }}".format(json_data)
    ...
    def OnTitleChanged(self, sender, e):
        ...
        try:
            if new_title == "SET_LOCATION":
                Rhino.RhinoApp.WriteLine("RhinoCharts: Initializing HUD repositioning...")
                self.pick_hud_location()
                return

            if new_title == "CLOSE_HUD":
                Rhino.RhinoApp.WriteLine("RhinoCharts: Closing HUD via Dashboard toggle...")
                if 'overlay_window' in sys.modules:
                    sys.modules['overlay_window'].ChartOverlay.close_instance()
                return

            if new_title.startswith("DUMP:"):
                # Handle the asynchronous data dump from the UI
                json_data = new_title[5:]
                Rhino.RhinoApp.WriteLine("RhinoCharts v1.0.99: DUMP received ({} chars).".format(len(json_data)))
                
                # EXECUTE HUD LOGIC DIRECTLY
                ...
                if not os.path.exists(html_path):
                    Rhino.RhinoApp.WriteLine("RhinoCharts ERROR: HTML file missing at " + html_path)
                    return

                def launch_hud():
                    try:
                        Rhino.RhinoApp.WriteLine("RhinoCharts: Reloading and Launching Overlay Window...")
                        # Force reload the overlay module to catch UI changes
                        if 'overlay_window' in sys.modules:
                            reload(sys.modules['overlay_window'])
                        import overlay_window
                        overlay_window.ChartOverlay.show_or_update(html_path, json_data)
                        Rhino.RhinoApp.WriteLine("RhinoCharts: HUD Launch Command Finished.")
                    except Exception as inner_ex:
                        Rhino.RhinoApp.WriteLine("RhinoCharts HUD EXECUTION ERROR: " + str(inner_ex))

                Rhino.RhinoApp.WriteLine("RhinoCharts: Dispatching to UI Thread...")
                from System import Action
                
                # Robust Fallback Chain for Rhino 7 Threading
                def execute_dispatch():
                    try:
                        # Method A: Standard Eto Instance
                        forms.Application.Instance.Invoke(Action(launch_hud))
                        return True
                    except:
                        try:
                            # Method B: Rhino App Global
                ...
                if not execute_dispatch():
                    Rhino.RhinoApp.WriteLine("RhinoCharts: All thread jumps failed. Forcing immediate execution...")
                    launch_hud()
                else:
                    Rhino.RhinoApp.WriteLine("RhinoCharts: Dispatch command sent successfully.")
                
            elif new_title.startswith("RELOAD"):
                Rhino.RhinoApp.WriteLine("RhinoCharts: Reloading Dashboard...")
                self.webview.Reload()
            elif new_title.startswith("ERROR:"):
                Rhino.RhinoApp.WriteLine("RhinoCharts UI Bridge Error: {}".format(new_title[6:]))
    ...
    def pick_hud_location(self):
        ...
        try:
            ...
            # Use standard RhinoGet.GetPoint for simple viewport picking
            res, pt = RhinoGet.GetPoint("Pick new HUD location (Top-Left corner)", False)
            ...
            # Update HUD Location via UI Thread
            def move_hud():
                try:
                    if 'overlay_window' in sys.modules:
                        ...
                        if overlay:
                            ...
                            Rhino.RhinoApp.WriteLine("RhinoCharts: HUD moved to [{}, {}]".format(screen_pt.X, screen_pt.Y))
                        else:
                            Rhino.RhinoApp.WriteLine("RhinoCharts: HUD must be visible to move it.")
                except Exception as ex:
