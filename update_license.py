...
def refactor_file(filepath):
    try:
        ...
        subprocess.run(["ruff", "check", "--fix", filepath], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        subprocess.run(["ruff", "format", filepath], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except FileNotFoundError:
        print("Warning: 'ruff' not found. Skipping auto-refactoring. To enable, run: pip install ruff")
...
def process_file(filepath):
    ...
    for line in lines:
        ...
        if stripped.startswith('#'):
            if should_keep_comment(stripped) or "GNU General Public License" in line or "This program is free software" in line:
