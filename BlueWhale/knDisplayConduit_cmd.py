...
class DynamicAreaConduit(Rhino.Display.DisplayConduit):
    ...
    def DrawForeground(self, e):
        ...
        e.Display.Draw2dText(
            "AREA AUDIT ACTIVE", Color.Black, Rhino.Geometry.Point2d(20, 20), False, 24
        )
        e.Display.Draw2dText(
            "Run command again to clear graphics.",
            Color.DarkGray,
            Rhino.Geometry.Point2d(20, 85),
            False,
            14,
        )
        ...
        for obj_id in self.object_ids:
            ...
            if isinstance(geom, Rhino.Geometry.Curve) and geom.IsClosed:
                ...
                geom,
                (Rhino.Geometry.Brep, Rhino.Geometry.Surface, Rhino.Geometry.Extrusion),
            ):
                amp = Rhino.Geometry.AreaMassProperties.Compute(geom)
                if amp:
                    area = amp.Area
            ...
            if area > 0:
                ...
                if bbox.IsValid:
                    center = bbox.Center
                    e.Display.DrawDot(
                        center, "{0:.1f} sf".format(area), Color.DarkOrange, Color.White
                    )

        e.Display.Draw2dText(
            "Total Area: {0:.2f} sf".format(total_area),
            Color.Blue,
            Rhino.Geometry.Point2d(20, 55),
            False,
            18,
        )
...
def RunCommand(is_interactive):
    ...
    object_ids = rs.GetObjects(
        "Select closed curves or surfaces to audit area dynamically",
