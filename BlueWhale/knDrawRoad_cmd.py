...
def RunCommand(is_interactive):
    ...
    values = ui_osm3d.show_eto_form_with_titles(
        "Draw Road Toolbox", "Width of Road", "Radius"
    )
    if not values:
    ...
    pts = rs.GetPoints(
        True,
        message1="Pick first point",
        message2="Pick next point or press Enter to end",
        max_points=99,
    )
    ...
    try:
        ...
        intersection_events = rg.Intersect.Intersection.CurveSelf(
            crv_obj, sc.doc.ModelAbsoluteTolerance
        )
        ...
        if len(pts) > 2:
            filleted_crv = rg.Curve.CreateFilletCornersCurve(
                crv_obj,
                radius,
