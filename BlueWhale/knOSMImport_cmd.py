...
from assets import osm_fetcher
...
def getWayPoints(children_id, model):
    ...
    for x in children_id:
        if x in lon and x in lat:
            result.append([osm_logic.lon_to_meters(lon[x]), osm_logic.lat_to_meters(lat[x]), 0])
    return result
...
def createCurve(waypoints, layer=None, attributes=None, create_3d=False):
    ...
    if create_3d and attributes and 'building' in attributes:
        ...
        if height > 0:
            ...
            if ext_height > 0:
                ext_id = rs.ExtrudeCurve(curve, rs.AddLine([0,0,0], [0,0,ext_height]))
                if ext_id:
                    if min_height > 0:
...
def RunCommand(is_interactive):
    ...
    source = rs.ListBox(["File", "Direct_Online"], "Select OSM Source", "OSM Data Source")
    if not source: return Rhino.Commands.Result.Cancel
    ...
    if source == "File":
        ...
        if not bbox_str: return Rhino.Commands.Result.Cancel
        
        try:
            bbox = tuple(map(float, bbox_str.split(",")))
            if len(bbox) != 4: raise ValueError
    ...
    try:
        ...
        # Calculate total elements for progress bar
        total_relations = sum(len(rel_dict) for rel_dict in model_elements['relation'].values())
        total_ways = len(model_elements['way'])
        total_elements = total_relations + total_ways
        
        progress_dialog = ui_progressform.ModelCreationProgress(total_elements, "Creating 3D Model...")
        ...
        for layer_info in layer_conditions:
            ...
            geometry_utilities.create_layer_if_missing(ln, layer_info.get("layer_color", (150,150,150)))
        ...
        for rel_type, rel_dict in model_elements['relation'].items():
            for id, relation in rel_dict.items():
                current_step += 1
                if current_step % 5 == 0:
                    progress_dialog.update(current_step, "Processing Relation {0}/{1}...".format(current_step, total_elements))
                ...
                for way_id in relation["children"]:
                    ...
                    waypoints = getWayPoints(model_elements["way"][way_id]["children"], osm_model)
                    layer_name = getLayerName(relation["attributes"])
                    curve = createCurve(waypoints, layer_name, relation["attributes"], create_3d)
                    if curve:
                        curves.append(curve)
                        created_ids.append(curve)
                
                if curves:
                    group_name = relation["attributes"].get("name", "OSM_Relation_{0}".format(id))
        ...
        for id, way in model_elements['way'].items():
            ...
            if current_step % 10 == 0:
                progress_dialog.update(current_step, "Processing Way {0}/{1}...".format(current_step, total_elements))
                
            if id in used_ways:
                continue
            waypoints = getWayPoints(way["children"], osm_model)
            attributes = way["attributes"]
            obj_id = createCurve(waypoints, getLayerName(attributes), attributes, create_3d)
            if obj_id:
        ...
        if created_ids:
            ...
            if bbox:
                xform, center_in_meters = osm_logic.calculate_localization_transform(bbox)
                ...
                rs.SetDocumentUserText("GeoLocated Origin", "{0}, {1}".format(center_lat, center_lon))
                rs.SetDocumentUserText("Model Location", "Local")
                print("OSM Data localized to origin. Center: {0}, {1} (Lat, Lon)".format(center_lat, center_lon))
