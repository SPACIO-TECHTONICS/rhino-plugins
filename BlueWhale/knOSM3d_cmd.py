...
def RunCommand(is_interactive):
    ...
    form_values = nf.show_eto_form_with_titles(
        "Extrude Buildings Toolbox", "Level Height"
    )
    if not form_values:
    ...
    if not rs.IsLayer(layer_name):
        ...
        if existing_objs:
            res = rs.MessageBox(
                "Do you want to delete the existing buildings on layer {0}?".format(
                    layer_name
                ),
                1,
                "Extrude Buildings Toolbox",
            )
            if res == 1:
