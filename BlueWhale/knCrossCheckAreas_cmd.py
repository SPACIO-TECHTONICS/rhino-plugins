...
def RunCommand(is_interactive):
    ...
    if mode == "EntireDocument":
        ...
        objects = rs.GetObjects("Select breps to check projected areas", filter=8|16, preselect=True)
        if not objects:
    ...
    # Set up temporary layer for mismatch dots
    ...
    for obj in objects:
        ...
        if planar_breps:
            ...
            if union_curves:
                planar_breps_union = Rhino.Geometry.Brep.CreatePlanarBreps(union_curves, tolerance)
        ...
        if area is not None:
            ...
            if diff <= tolerance:
                print("MATCH: {0} (Expected: {1}, Calculated XY: {2:.4f})".format(name, expected_area, area))
            else:
                print("MISMATCH: {0} (Expected: {1}, Calculated XY: {2:.4f})".format(name, expected_area, area))
    ...
    if created_dots:
        rs.Redraw()
        rs.GetString("{0} mismatches found! Press Enter/Space to clear text dots...".format(len(created_dots)))
