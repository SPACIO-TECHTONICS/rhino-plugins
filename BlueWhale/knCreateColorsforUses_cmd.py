...
class ColorForm(forms.Form):
    ...
    def show_dialog(self, title, keys, values):
        ...
        for i in range(len(keys)):
            ...
            try:
                ...
                button.BackgroundColor = drawing.Color.FromArgb(
    ...
    def OnPushPickButton(self, sender, e):
        name_box = sender.Tag
        Rhino.UI.EtoExtensions.PushPickButton(
            self, lambda s, a: self.AddByPicking(name_box)
        )
        ...
        for obj in objs:
            ...
            if u_type:
                ...
                if color_str:
                    try:
                        color = tuple(
                            int(x) for x in color_str.split(",") if x.strip().isdigit()
                        )
                        if len(color) == 3:
...
def RunCommand(is_interactive):
    ...
    if matches:
        ...
        rs.MessageBox(
            "Initialized types with default colors. Please run the command again to manage them."
        )
        return Rhino.Commands.Result.Success
